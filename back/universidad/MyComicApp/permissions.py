...
def create_groups_and_permissions():
    ...
    # Asignar permisos al grupo 'Vendedor'
    vendedor_perms = order_perms | product_perms | category_perms | user_perms.exclude(codename__in=['add_user', 'change_user', 'delete_user'])
    vendedor_group.permissions.set(vendedor_perms)

    # Asignar permisos directamente a los usuarios basados en su rol
    for user in User.objects.all():
        if user.role and user.role.name == 'Vendedor':
            user.user_permissions.set(vendedor_perms)

    # Asociar el superusuario al grupo 'Vendedor' (Opcional, si quieres que superusuario tenga esos permisos también)
    superuser = User.objects.filter(is_superuser=True).first()
    if superuser:
        superuser.groups.add(vendedor_group)
